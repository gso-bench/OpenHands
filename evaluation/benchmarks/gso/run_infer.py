...
def get_config(
    ...
    logger.info(f'Using GSO image: {base_container_image}')

    sandbox_config = get_default_sandbox_config_for_eval()
    sandbox_config.base_container_image = base_container_image
    sandbox_config.enable_auto_lint = True
    sandbox_config.use_host_network = False
    sandbox_config.timeout = 20 * 60  # large timeout per action for rebuilds/tests
    sandbox_config.platform = 'linux/amd64'  # Add platform to solve issue 4401
...
if __name__ == '__main__':
    ...
    run_evaluation(
        ...
        process_instance,
        timeout_seconds=3 * 60 * 60,  # 3 hour PER instance should be more than enough
        max_retries=1,
    )
